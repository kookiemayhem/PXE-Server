...
###Making Directory for Temp. Config Files
mkdir -p ~/pxe-files/backup

printf "\n\e[34m Backing Up Current Files…\e[0m"
sudo cp /etc/default/tftpd-hpa ~/pxe-files/backup
sudo cp /etc/exports ~/pxe-files/backup
sudo cp /etc/dhcp/dhcpd.conf ~/pxe-files/backup
sudo cp /etc/network/interfaces ~/pxe-files/backup
sudo cp /etc/xinetd.d/tftp ~/pxe-files/backup
sudo cp /var/lib/tftpboot/pxelinux.cfg/default ~/pxe-files/backup
printf "\e[32mDone. \n Coppied To pxe-files/backup\e[0m"
...
sudo cp ~/pxe-files/interfaces /etc/network/interfaces
...
sudo cp ~/pxe-files/dhcpd.conf /etc/dhcp/dhcpd.conf
printf "\e[32mDone.\e[0m"
...
sudo cp ~/pxe-files/tftp /etc/xinetd.d/tftp
printf "\e[32mDone.\e[0m"
...
sudo cp ~/pxe-files/tftpd-hpa /etc/default/tftpd-hpa
printf "\e[32mDone.\e[0m"
...
#Adding Boot Files from ISO
sudo mount -o loop ~/$isoimg /mnt/ >/dev/null
sudo cp -r /mnt/* $tftproot/$bootfiles
sudo cp -r /mnt/.disk $tftproot/$bootfiles
sudo cp /mnt/casper/initrd.lz /mnt/casper/vmlinuz.efi $tftproot/$kernel/
sudo umount /mnt/
printf "\n\e[32m Added Boot Files From ISO\e[0m"
...
sudo cp ~/pxe-files/default $tftproot/pxelinux.cfg && printf "\e[32mDone.\e[0m"
...
sudo mv ~/pxe-files/exports /etc/exports
sudo exportfs -a
sudo /etc/init.d/nfs-kernel-server start >/dev/null
printf "\e[32mDone.\e[0m"
printf "\n\e[32m PXE is Now Ready \e[0m\n\n\n"
